...
class Ui:
    ...
    def __init__(self):
        ...
        discipline_repository = ParseProperties.find_discipline_repo()
        student_repository = ParseProperties.find_student_repo()
        grade_repository = ParseProperties.find_grade_repo()

        self.__discipline_service = DisciplineService(discipline_repository, grade_repository, self.undo_service)
        self.__student_service = StudentService(student_repository, grade_repository, self.undo_service)
        self.__grade_service = GradeService(grade_repository, self.undo_service)
        ...
        try:
            ...
            if chosen_option == DISPLAY_STUDENTS_WITH_BEST_SCHOOL_SITUATION:
                ...
                for student_id_and_grade in best_students_id:
                    print("Mean: " + str(student_id_and_grade[CORRESPONDING_GRADE]))
                    print(str(self.__student_service.get_student_by_id(student_id_and_grade[CORESPONDING_ID])))
                    print("---------------------------------------------")
            if chosen_option == DISPLAY_DISCIPLINES_SORTED_BY_AVERAGE:
                ...
                for discipline_id_and_mean in disciplines_id_and_mean:
                    print("Mean: " + str(discipline_id_and_mean[CORRESPONDING_GRADE]))
                    print(str(self.__discipline_service.get_discipline_by_id(discipline_id_and_mean[CORESPONDING_ID])))
                    print("---------------------------------------------")
